  ...
  return (
    <section style={{ background: `linear-gradient(135deg,#3a1a00 0%,${BROWN} 60%,${BROWN2} 100%)`, position:'relative', overflow:'hidden', fontFamily:'Georgia,serif' }}>
      <style>{`
        ...
        .hl-icon { position:relative; z-index:1; width:56px; height:56px; border-radius:50%; border:1.5px solid rgba(229,190,16,0.5); background:rgba(229,190,16,0.1); display:flex; align-items:center; justify-content:center; transition:border-color 0.3s,background 0.3s; }
        .hl-cell:hover .hl-icon { border-color:#e5be10; background:rgba(229,190,16,0.2); }
      `}</style>
      ...
      <div style={{ textAlign:'center', padding:'36px 24px 0' }}>
        <div style={{ display:'inline-flex', alignItems:'center', gap:10, marginBottom:12 }}>
          ...
          <span style={{ fontSize:11, fontWeight:700, letterSpacing:'0.15em', textTransform:'uppercase', color:'rgba(229,190,16,0.7)', fontFamily:'sans-serif' }}>Why Choose Us</span>
          <div style={{ height:1, width:44, background:'linear-gradient(90deg,rgba(229,190,16,0.6),transparent)' }} />
        </div>
        <h2 style={{ fontSize:'clamp(26px,4vw,38px)', fontWeight:800, color:GOLD, lineHeight:1.1, margin:'0 0 8px' }}>College Highlights</h2>
      ...
      <div style={{ display:'flex', marginTop:32 }}>
        {highlights.map((item, idx) => (
          ...
          >
            ...
            <div style={{ position:'absolute', inset:0, background:'linear-gradient(135deg,rgba(58,26,0,0.75),rgba(58,26,0,0.5))', pointerEvents:'none' }} />
            ...
            <h3 style={{ position:'relative', zIndex:1, fontSize:17, fontWeight:800, color:GOLD, letterSpacing:'0.02em' }}>{item.title}</h3>
            <p style={{ position:'relative', zIndex:1, fontSize:13, color:'rgba(253,248,238,0.8)', lineHeight:1.7, fontFamily:'sans-serif', fontWeight:400, maxWidth:220 }}>{item.description}</p>
          </motion.div>
        ))}
      </div>
      <div className="hl-gold-bar" />
